...
def evaluate_explanations(model, clean_model, dataloader, attack, target_fn, device):
    ...
    for images, labels in dataloader:
        ...
        cams_model = normalize_cam(cams_model)

        # metrics clean
        mse_clean.extend(((cams_model - cams_ref)**2).mean(dim=(1,2)).detach().cpu().numpy())
        ...
        cams_trig = normalize_cam(cams_trig)
        ...
        # metrics trigger
        mse_trigger.extend(((cams_trig - target)**2).mean(dim=(1,2)).detach().cpu().numpy())
